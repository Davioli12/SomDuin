...
class AppAPI:
    ...
    def adicionar_widget(self, widget_fn):
        self._app.after(
            0,
            lambda: self._app._ext_manager.montar_widget(widget_fn)
        )
    ...
    def toast(
        ...
        self._app.after(
            0,
            lambda: self._app._mostrar_toast(msg, cor, duracao_ms)
        )

    def log(self, msg: str):
        self._app.after(
            0,
            lambda m=msg: self._app._ext_manager.log(m)
        )
...
class _ExtThread(threading.Thread):
    ...
    def _chamar_com_timeout(self, fn, *args, timeout=WATCHDOG_TIMEOUT):
        ...
        if t.is_alive():
            print(
                f"[watchdog] "
                f"{self.ext.NOME}.{fn.__name__} excedeu {timeout}s"
            )
...
class ExtensionManager:
    ...
    def __init__(
        ...
        self._loaded: dict[
            str,
            tuple[BaseExtension, _ExtThread]
        ] = {}
    ...
    def descobrir(self) -> list[dict]:
        ...
        for f in sorted(os.listdir(self._ext_dir)):
    ...
    def _ler_metadados(self, arquivo: str) -> dict:
        ...
        path = os.path.join(self._ext_dir, arquivo)
    ...
    def carregar(self, arquivo: str) -> bool:
        ...
        try:
            ...
            self._app.after(
                0,
                lambda e=ext: self._montar_widget_ext(e)
            )
